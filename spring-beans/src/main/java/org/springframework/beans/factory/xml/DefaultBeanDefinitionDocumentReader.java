		...
		if (delegate.isDefaultNamespace(root)) {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					// <1> 如果该节点使用默认命名空间，执行默认解析 如：配置文件式声明<bean id="studentService" class="org.springframework.core.StudentService" />
					if (delegate.isDefaultNamespace(ele)) {
						parseDefaultElement(ele, delegate);
					}
					// 如果该节点非默认命名空间，执行自定义解析 如：自定义注解方式：<tx:annotation-driven>
					else {
